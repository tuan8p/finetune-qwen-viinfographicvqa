...
import torch
import wandb
from huggingface_hub import login # Bổ sung import để login Hugging Face
...
def merge_model(args, adapter_path):
    ...
    # ===== BỔ SUNG LOGIC ĐẨY LÊN HUGGING FACE =====
    if args.push_to_hub:
        print("\n[Bước 4] Đẩy model lên Hugging Face Hub...")
        if not args.hf_repo_id:
            print("LỖI: Bạn cần cung cấp --hf_repo_id (ví dụ: username/qwen-vl-finetuned) để đẩy model.")
            return
        
        # Đăng nhập nếu token được cung cấp qua CLI
        if args.hf_token:
            login(token=args.hf_token)
            
        print(f"Đang đẩy weights của model lên {args.hf_repo_id}...")
        merged_model.push_to_hub(args.hf_repo_id, safe_serialization=True)
        
        print("Đang đẩy processor...")
        processor.push_to_hub(args.hf_repo_id)
        
        print(f"Thành công! Bạn có thể xem model tại: https://huggingface.co/{args.hf_repo_id}")
...
def main():
    ...
    parser.add_argument("--push_to_hub", action="store_true", help="Bật cờ này để đẩy model lên HF Hub sau khi merge")
    parser.add_argument("--hf_repo_id",  type=str, default=None, help="ID của repo trên Hugging Face (ví dụ: username/my-qwen-model)")
    parser.add_argument("--hf_token",    type=str, default=None, help="Hugging Face Write Token (Cần thiết nếu chưa chạy 'huggingface-cli login')")

    args = parser.parse_args()

    # W&B Initialization Logic
    if args.use_wandb:
        if args.wandb_api_key:
            wandb.login(key=args.wandb_api_key)
        
        project_name = args.wandb_project if args.wandb_project else "vqa-qlora"
        run_name = args.wandb_run_name if args.wandb_run_name else f"{args.task}_run"
...
python main.py \
    ...
    --root_dir /data \
    --push_to_hub \
    --hf_repo_id "username_cua_ban/ten_model_moi" \
    --hf_token "hf_XXXXXXX" \
    --use_wandb \
    --wandb_project ... \
    --wandb_run_name .. \
